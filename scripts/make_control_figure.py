...
chosen threshold is the whole engineering argument.
...
# Categorical slots 1 and 2 of the validated palette. Checked with
# scripts/validate_palette.js: all six checks pass on the light surface
# (worst adjacent CVD dE 24.7 protan, normal-vision dE 33.6).
RISK = "#2a78d6"
ALARM = "#eb6834"
INK = "#0b0b0b"
MUTED = "#52514e"
...
VERDICT = {
    "metal_nut": "one clean part in 22 escalated: the guarantee is affordable",
    "grid": "every clean part escalated: the guarantee automates nothing",
}


def panel(ax, cat: str) -> None:
    ...
    ax.annotate(f"{far:.0%} of clean parts flagged\n({area_txt} of the frame)",
                xy=(thr, far), xytext=(0.42, far + (0.14 if far < 0.6 else -0.26)),
                fontsize=8.4, color=INK,
                arrowprops=dict(arrowstyle="-", color=MUTED, linewidth=0.9))

    ax.set_title(f"{cat}   ·   {VERDICT[cat]}", fontsize=10, color=INK, pad=6)
...
def main() -> int:
    ...
    for ax, cat in zip(axes, ["metal_nut", "grid"]):
        panel(ax, cat)
    ...
    fig.suptitle("The curve conformal calibration reads, and the one it does not",
                 fontsize=12, y=0.975, color=INK)
    fig.text(0.5, 0.088,
             "Lowering λ to stop missing defects can only flag more pixels, so the two "
             "rates move in opposite directions. Calibration is blind to the orange curve: "
             "a defect-free part has no defect pixels to miss.",
