...
- Conformal layer: risk control over a threshold grid, taking the **largest**
  threshold whose finite-sample-corrected risk meets α. The loss is per-image
  FNR, the fraction of true defect pixels the mask misses, which is
  nondecreasing in the threshold and therefore admits the monotone construction
  of Angelopoulos et al. (2022).
...
| `metal_nut` | 0.130 conformal | 0.644 | **0.055 ✓** | 0.207 | **0.045** (1 of 22) |
...
- **The guarantee is one-sided by construction.** FNR is the fraction of *true
  defect pixels* missed, so a defect-free part contributes zero loss whatever
  the mask does. Nothing in the conformal machinery constrains false alarms, and
  the threshold it picks is lower than 0.5, which can only flag more. The
  defect-free control split exists because the theory offers no warning here.
- **`grid` meets its guarantee and is useless.** At λ̂ = 0.09 it escalates every
  defect-free part, so its contribution to line throughput is zero. Retraining at
  640 px raised validation IoU from 0.068 to 0.359 and cut naive false alarms
  from 0.429 to 0.095, and moved the calibrated false-alarm rate not at all. A
  pixel-level FNR target is the wrong contract for hairline defects at any
  resolution tested.
